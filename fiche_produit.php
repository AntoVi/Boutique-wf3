...
if(isset($_SESSION['tabIdArticle']))
{
    $positionId = array_search($_GET['id_article'], $_SESSION['tabIdArticle']);

    if($positionId == false)
    {
        array_push($_SESSION['tabIdArticle'], $_GET['id_article']);
    }
    ...
    $arrayJson = json_encode($_SESSION['tabIdArticle']);
...
?>
    ...
    <?php if(isset($msg)): ?>
        <p class="bg-danger col-md-4 mx-auto p-3 text-center text-white my-3 disparition"><?= $msg; ?></p>
    <?php endif; ?>
    ...
    <div class="row mb-5">
        <div class="bg-white shadow-sm rounded d-flex zone-card-fiche-produit">
            ...
            <a href="<?= $product['photo']; ?>" data-lightbox="image" data-title="<?= $product['titre'] ?>" data-alt="<?= $product['titre']; ?>" class="d-flex flex-column justify-content-center">
                ...
                <small class="text-success text-center fst-italic">Cliquer pour agrandir</small>
